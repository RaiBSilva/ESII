...
for i in range(1,len(PdfFile)):
    colunas = PdfFile[i].columns
    funcionarios.append((colunas[0], colunas[1], colunas[2], colunas[3], colunas[4], colunas[5], colunas[6], colunas[7], colunas[8], colunas[9]))
    for row in range(0, len(PdfFile[i])):
        tupla = PdfFile[i].iloc[row]
        funcionarios.append((tupla[0], tupla[1], tupla[2], tupla[3], tupla[4], tupla[5], tupla[6], tupla[7], tupla[8], tupla[9]))
